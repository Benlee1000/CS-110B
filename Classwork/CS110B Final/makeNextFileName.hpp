...
string makeNextFileName() {
    ...
    if (mm == -1) {
        ...
        while (true) {
	        ...
	        //mp it would not be hard to include the C++ internal error
                cout << "makeNextFileName(): \"" << mmStr << "\" is non-numeric for month\n"
		     << "    C++ internal error: " << e.what() << endl;
                validationFail = true;
            } // catch
            try { dd = stoi(ddStr); }
            catch (exception &e) {
