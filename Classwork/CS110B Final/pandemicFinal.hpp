...
const int CSV_DATE = 0;  //mp repurpose to show csv file date
const int COUNTY = 1;
...
const string Terra = "Terra";  //Terra == Earth, our planet
const string US = "US";
...
class Grouping {
protected:
    string county;   //NULL_STRING or BA county name
    string province; //NULL_STRING or California
    string country;   //US or CANADA
    float longtitude;
    float latitude;
    long population;  //set from config file
 ...
 public:
    ...
    Grouping () {};
    Grouping (string county, string province, string country, float longtitude, float latitude, int population);
...
public:
    ...
    Aggregator(Aggregator *parent, string planet = "", string country = "", string province = "",string region = "");
...
class CSV {
    ifstream csv_file;
    string fileName;    //mp new field for final
    string curLine;
    vector<string> fields;  //Comma Separated Values
  
public:
    CSV (string fn) {
        this->fileName = fn;                 //mp new lines for final
        this->csv_file.open(this->fileName); //mp   
    ...
    static void makeFields(string text, string fieldDelim, vector<string> *fields);

    /*
    * new function 
    *Uses the CSV file name, which has the form
    * MM-DD-YYYY.csv, to create a date with the format
    * YYYY-MM-DD and store it in the CSV_DATE record in
    * this->fields
    *
    * You will need to call setCsvDate at an appropriate part of the processing
    * so that the Sample objects have the date of the CSV file.
    */
    void setCsvDate() {
        vector<string> dateFields;

        CSV::makeFields(this->fileName,DOT,&dateFields);
        string csvDate = dateFields[0];
        dateFields.clear();
        //complete this function to set CSV_DATE field in the fields vector -DONE
        CSV::makeFields(csvDate, DASH, &dateFields);
        string singleMM = dateFields[0];
        string singleDD = dateFields[1];
        string singleYYYY = dateFields[2];
        dateFields.clear();
        string NewDate = singleYYYY + DASH + singleMM + DASH + singleDD;
        this->fields[CSV_DATE] = NewDate;
        return;
    } //setCsvDate()
    ...
    static string makeDateStr(vector<string> f) {
        //mp int blankPos = f[LAST_UPDATE].find(BLANK,0);
        //mp return(f[LAST_UPDATE].substr(0,blankPos));
        return f[CSV_DATE];
    }//makeDateStr()
